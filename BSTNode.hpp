...
public:
  ...
  BSTNode<Data>* left;
  BSTNode<Data>* right;
  Data data;  // the const Data in this node
   ...
   */ // TODO
  //BSTNode<Data>* successor(int key, BSTNode<Data>* node) const {
  BSTNode<Data>* successor() const {	  
        if ( right ) {
          ...
          BSTNode<Data>* y = parent;
          BSTNode<Data>* x;
          while(y!=0 && this == y->right) {
            x = y;
            y = x->parent;
          }
          return y;
...
template <typename Data>
std::ostream& operator<<(std::ostream& stm, BSTNode<Data> & n) {
  stm << '[';
